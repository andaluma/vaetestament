---
title: "Probate in Dubai voor Nederlandse erfgenamen: het proces in detail"
slug: "probate-dubai-nederlandse-erfgenamen"
meta_description: "Wat doorloopt u als Nederlandse erfgenaam bij probate in Dubai? Stappen, documenten, kosten (EUR 10.000+) en tijdlijn (12-24 maanden) in detail."
keywords:
  - probate Dubai Nederlands
  - Dubai probate proces erfgenamen
  - nalatenschap Dubai kosten
  - probate UAE tijdlijn
  - erfgenaam Dubai vastgoed
publish_date: 2026-06-23
status: draft
author: André van Wijngaarden
schema_type: Article
---


Als een Nederlandse vastgoedeigenaar in de UAE overlijdt zonder DIFC Will, starten de erfgenamen een probate-procedure bij de UAE-rechtbank. Dit traject kost doorgaans meer dan EUR 10.000 en duurt 12 tot 24 maanden. Dit artikel beschrijft het volledige proces: elke stap, elk document en elke kostenpost.

## Waarom probate?

Vastgoed in de UAE kan niet zomaar overgeschreven worden na overlijden. Zonder geldig lokaal testament moet een rechtbank vaststellen wie de rechtmatige erfgenamen zijn en hoe het vastgoed verdeeld wordt. Die procedure heet probate.

Voor Nederlandse erfgenamen betekent dit navigeren door een buitenlands rechtssysteem, in het Arabisch, met andere procesregels dan zij gewend zijn.

## Het proces stap voor stap

### Stap 1: overlijdensakte verkrijgen en legaliseren

De Nederlandse overlijdensakte moet worden voorzien van een apostille (legalisatie voor internationaal gebruik). Dit loopt via de rechtbank in Den Haag. Vervolgens moet de akte beëdigd worden vertaald naar het Arabisch door een gecertificeerde vertaler.

**Doorlooptijd:** 2 tot 4 weken.
**Kosten:** EUR 200 tot EUR 500 (apostille + vertaling).

### Stap 2: verklaring van erfrecht

In Nederland moet een notaris een verklaring van erfrecht opstellen. Dit document bewijst wie de wettelijke erfgenamen zijn. Ook dit document moet geapostilleerd en beëdigd vertaald worden.

**Doorlooptijd:** 2 tot 6 weken (afhankelijk van de notaris en eventuele complicaties).
**Kosten:** EUR 500 tot EUR 1.500 (notaris + apostille + vertaling).

### Stap 3: volmacht voor lokale vertegenwoordiging

De erfgenamen moeten een advocaat in de UAE machtigen om namens hen op te treden. Daarvoor is een notariële volmacht nodig, eveneens geapostilleerd en vertaald.

**Doorlooptijd:** 1 tot 2 weken.
**Kosten:** EUR 200 tot EUR 500.

### Stap 4: inschakelen van een UAE-advocaat

Een lokale advocaat dient de probate-aanvraag in bij de bevoegde rechtbank. De keuze van rechtbank hangt af van het emiraat waar het vastgoed staat.

**Kosten advocaat:** EUR 3.000 tot EUR 8.000, afhankelijk van de complexiteit en het emiraat. Sommige kantoren werken met uurtarieven, andere met vaste prijzen.

### Stap 5: rechtbankprocedure

De rechtbank beoordeelt de ingediende documenten, stelt de erfgenamen vast en beslist over de verdeling van het vastgoed. Recente wijzigingen in de UAE-regelgeving hebben de standaard erfregels voor buitenlandse eigenaren aangepast, maar de toepassing kan per geval en per emiraat verschillen.

**Doorlooptijd:** 3 tot 12 maanden (vanaf indiening). Vertragingen door aanvullende documentatieverzoeken zijn gebruikelijk.

### Stap 6: eigendomsoverdracht

Na de rechtbankuitspraak wordt het vastgoed formeel overgeschreven bij de land authority van het betreffende emiraat. Dit vereist aanvullende registratiekosten en administratieve stappen.

**Doorlooptijd:** 2 tot 4 weken na de uitspraak.
**Kosten:** variabel, doorgaans EUR 500 tot EUR 2.000 (registratiekosten land authority).

## Totaaloverzicht kosten

| Kostenpost | Indicatie |
|---|---|
| Apostilles en vertalingen | EUR 600 tot EUR 1.500 |
| Verklaring van erfrecht (NL) | EUR 500 tot EUR 1.500 |
| Volmacht en legalisatie | EUR 200 tot EUR 500 |
| UAE-advocaat | EUR 3.000 tot EUR 8.000 |
| Rechtbankkosten | EUR 500 tot EUR 1.000 |
| Eigendomsoverdracht | EUR 500 tot EUR 2.000 |
| **Totaal** | **EUR 5.300 tot EUR 14.500+** |

De ondergrens geldt voor een overzichtelijke situatie met één eigendom in Dubai. Bij meerdere eigendommen, eigendommen in verschillende emiraten of geschillen tussen erfgenamen lopen de kosten op.

## Totaaloverzicht tijdlijn

| Fase | Indicatie |
|---|---|
| Documentatie in Nederland | 4 tot 10 weken |
| UAE-advocaat en indiening | 2 tot 4 weken |
| Rechtbankprocedure | 3 tot 12 maanden |
| Eigendomsoverdracht | 2 tot 4 weken |
| **Totaal** | **6 tot 18 maanden** (realistisch: 12 tot 24 maanden) |

## Wat gedurende die periode gebeurt met het vastgoed

Zolang de probate-procedure loopt, is het vastgoed effectief bevroren. De erfgenamen kunnen het niet verkopen, niet verhuren aan nieuwe huurders en niet bezwaren met een hypotheek. Bestaande huurcontracten lopen door, maar de huuropbrengsten kunnen geblokkeerd zijn totdat het eigendom formeel is overgedragen.

## Het alternatief: een DIFC Will

Een DIFC Will voorkomt het volledige probate-traject. Het testament is geregistreerd bij het DIFC Wills Service Centre en wordt uitgevoerd via het DIFC-rechtssysteem. De procedure is sneller, goedkoper en verloopt in het Engels.

De kosten van een DIFC Will via VAEtestament: EUR 497, EUR 797 of EUR 1.499. Een fractie van de probate-kosten. Lees meer over de [pakketten en wat elk tier omvat](/wat-kost-een-difc-will).

## Veelgestelde vragen

### Kan ik als erfgenaam zelf naar Dubai reizen om het proces te versnellen?

Fysieke aanwezigheid versnelt het proces niet wezenlijk. De doorlooptijd wordt bepaald door de documentatieverwerking en de rechtbank-agenda, niet door de locatie van de erfgenamen.

### Wat als er meerdere erfgenamen zijn die het niet eens zijn?

Geschillen tussen erfgenamen compliceren de procedure aanzienlijk. De rechtbank kan aanvullend bewijs en documentatie verlangen, wat de doorlooptijd verder verlengt.

### Kan een Nederlandse notaris het probate-proces in de UAE afhandelen?

Nee. Een Nederlandse notaris heeft geen bevoegdheid in de UAE. U heeft een lokale UAE-advocaat nodig die bevoegd is om bij de betreffende rechtbank te procederen.

### Wat als het vastgoed in Abu Dhabi staat in plaats van Dubai?

Het probate-proces verloopt dan via de rechtbank in Abu Dhabi in plaats van Dubai. De stappen zijn vergelijkbaar, maar de doorlooptijd en kosten kunnen afwijken. Een DIFC Will dekt vastgoed in alle emiraten.

### Is probate te vermijden als de eigenaar al een Nederlands testament had?

Een Nederlands testament wordt in de UAE doorgaans niet erkend voor vastgoed op UAE-grond. De probate-procedure is dan nog steeds nodig. Alleen een lokaal geldig testament (zoals een DIFC Will) voorkomt dit. Lees meer over [de combinatie van beide testamenten](/difc-will-of-nederlands-testament).

### Wat als de overledene huurinkomsten ontving uit het vastgoed?

De huurinkomsten kunnen geblokkeerd worden totdat het eigendom formeel is overgedragen. Dit hangt af van de beheerstructuur en de bank waar de inkomsten binnenkomen. Het is een van de minder zichtbare gevolgen van een probate-procedure.

---

Voor een directe inschatting van uw situatie en het juiste pakket: [VAEtestament.nl](https://vaetestament.nl)
